...
barplot(vetor_total_resultados, col = c(1,2,3,4,5)) # 1 a 5 sao as cores. existem varias
...
# Adiciona smooth (margem de erro para cima ou para baixo)
ggplot(data, aes(x=my_x, y=my_y)) +    
  geom_point(shape=1) +  
  geom_smooth(method=lm , color="red", se=TRUE) # <- True adiciona o smooth 
